...
public:
    ...
    */
    ...
    void Lidar_MotionCalibration(
            ...
            tf::Stamped<tf::Pose> frame_end_pose,    // Equal to frame_mid_pose in Lidar_Calibration
            ...
            int& beam_number)
    {
       ...
       // 得到起始及终点位姿旋转角度的四元数表达方式
       tf::Quaternion start_Qua = frame_start_pose.getRotation();      // Slerp
       tf::Quaternion end_Qua = frame_end_pose.getRotation();

       // 得到起始及终点Odom坐标系下的XYZ坐标
       tf::Vector3 start_XYZ = frame_start_pose.getOrigin();    // lerp
       tf::Vector3 end_XYZ = frame_end_pose.getOrigin();
       ...
       double ratio_inc = 1.0 / (beam_number - 1.0);    // 比例的增量


       // 当前激光发射器在Odom坐标系下的位姿Placeholder 
       tf::Vector3 cur_XYZ;
       double cur_yaw;

       // 当前激光点在Odom坐标系下的位姿Placeholder
       double cur_X;
       double cur_Y;

       for(int i = 0; i < beam_number ; i++){
            int index = startIndex + i;
            cur_XYZ =  start_XYZ.lerp(end_XYZ, i*ratio_inc);       // Odom坐标系
            cur_yaw = tf::getYaw(start_Qua.slerp(end_Qua, i*ratio_inc)); // Odom坐标系

            // 计算当前激光坐标系下的激光点坐标
            double x = ranges[index] * cos(angles[index]);     // 当前激光坐标系
            double y = ranges[index] * sin(angles[index]);    // 当前激光坐标系
            
            // 计算Odom坐标系下的激光点坐标
            cur_X = x * cos(cur_yaw) - y * sin(cur_yaw) + cur_XYZ.x();  // Odom坐标系
            cur_Y = x * sin(cur_yaw) + y * cos(cur_yaw) + cur_XYZ.y();  // Odom坐标系

            Eigen::Vector3d point_odom(cur_X, cur_Y, 1);     

            // 计算在Base坐标系下的激光点坐标
            Eigen::Vector3d point_base = Tbo * point_odom;
            double x_base = point_base(0);
            double y_base = point_base(1);

            // 矫正range和angle
            ranges[index] = sqrt(x_base*x_base + y_base*y_base);
            angles[index] =  atan2(y_base,x_base);
       }
       //end of TODO
    }
    ...
    */
                           ...
                           tf::TransformListener * tf_)
    {
        ...
        for(int i = 0; i < beamNumber; i++)
        {
            ...
            double mid_time = start_time + time_inc * (i - start_index);  // time_inc 为每束激光数据的时间间隔
